...
| Time-domain data | Every cycle at full resolution (1 Hz `single`) | ~240 points per trace, only for the plotted cases (13 drive cycles + 9 initial-SOC + 6 offset traces since 2026-08-29); always includes the max-error sample. **Dropped**: full-resolution traces for all 144 cycles |
...
3. **Full-resolution traces** — the old ZIP gave every cycle at 1 Hz; we keep 240-point plots for the key cases. If researchers need raw traces for papers, add a per-submission download (storage cost ~1–2 MB per submission).
