...
| Evaluation (Python **and** MATLAB packages) | **Alliance Cloud VM** on Arbutus (`Ahmad-Development-Server`, 8 vCPU / 12 GB) running `socbench-worker.service`; Python in the `socbench-eval` sandbox, MATLAB R2026a in the `socbench-eval-matlab` sandbox licensed through MathWorks online licensing with the lab's campus-wide licence — see the VM runbook in `socbench-internal` | free (Alliance allocation + campus MATLAB licence) |
