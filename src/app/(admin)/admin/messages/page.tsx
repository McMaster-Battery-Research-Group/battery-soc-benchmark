          ...
          {msgs.map((m) => (
            <li key={m.id} className={`card p-4 ${m.resolved ? "opacity-60" : ""}`}>
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div>
                  <p className="font-heading font-semibold text-ink">{m.subject}</p>
                  <p className="text-xs text-grey-600">{m.name} · <a href={`mailto:${m.email}`} className="text-maroon underline">{m.email}</a> · {fmtDateTime(m.createdAt)}</p>
                </div>
