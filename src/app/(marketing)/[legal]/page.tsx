...
const PAGES: Record<string, { title: string; body: { h: string; p: string[] }[] }> = {
  terms: {
    ...
    body: [
      ...
      { h: "Analytics and end-user disclosures", p: ["Our site uses Google Analytics to collect visitation and interaction information, such as page visits, link and button clicks, and aggregate evaluation outcome events that include the selected model type and whether an evaluation succeeded or failed."] },
...
export default async function LegalPage({ params }: { params: Promise<{ legal: string }> }) {
  ...
  return (
    <>
      <PageHeader title={page.title} description="Last updated September 2026." />
