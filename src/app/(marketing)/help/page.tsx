...
const SECTIONS: { title: string; items: { q: string; a: React.ReactNode }[] }[] = [
  {
    ...
    items: [
      { q: "What does the submission process look like?", a: <>Create an account and verify your email. Package your estimator as a .zip (see <Link href="/docs#submission-format" className="text-maroon underline">submission format</Link>), run the local test tool, then upload it on the <Link href="/submit" className="text-maroon underline">Submit</Link> page with a name, description and model type. The site checks the archive structure immediately and queues the evaluation; you can watch progress on the submission page and will get an email when it finishes.</> },
      { q: "Which files are valid?", a: <>Only a .zip containing <code>Model.m</code>, <code>Model.p</code> or <code>Model.py</code> and <code>Settings.xlsx</code> at the top level, plus any supporting files. Sub-folders, renamed files, or missing Settings fields are rejected before upload completes. The default size limit is 50 MB.</> },
      ...
      { q: "My evaluation failed. What now?", a: <>Open the submission: the failure message and the evaluator log are shown to you. Typical causes are a NaN output, an error on the first call (initialisation when <code>nargin &lt; 2</code>), a missing supporting file, or use of an unavailable toolbox. Fix the package, run the local test tool, and use <em>Re-run</em> or submit again.</> },
