      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {selected.map((s, i) => (
          <div key={s.id} className="card flex items-start gap-3 p-4" style={{ borderTopColor: SERIES[i], borderTopWidth: 3 }}>
            ...
            <button onClick={() => { sendGAEvent("event", "button_click", { button_name: "remove_comparison_model" }); apply(ids.filter((x) => x !== s.id)); }} className="rounded-brand p-1 text-grey-500 hover:bg-grey-100 hover:text-ink" aria-label={`Remove ${s.modelName}`}><X className="size-4" /></button>
          </div>
        ))}
